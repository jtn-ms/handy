...
def upload():
    ...
    usedefault = True if any(argv.startswith('--default') for argv in sys.argv) else False
    if any(argv.startswith('--mail') for argv in sys.argv): send_mail(files=[sys.argv[1]],subject=sys.argv[1],smode=usedefault)
    else:
        ...
        send_mail(subject=result,smode=usedefault)
...
def download():
    ...
    usedefault = True if any(argv.startswith('--default') for argv in sys.argv) else False
    when,who,what=read_email(smode=usedefault)
